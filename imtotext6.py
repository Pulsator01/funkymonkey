...
# 2. Convert the image to LAB color space for better contrast enhancement
lab_image = cv2.cvtColor(resized_image, cv2.COLOR_BGR2LAB)

# Split the LAB image into L, A, and B channels
l_channel, a_channel, b_channel = cv2.split(lab_image)

# Apply histogram equalization to the L (lightness) channel to increase contrast
...
enhanced_image = cv2.cvtColor(lab_image_equalized, cv2.COLOR_LAB2BGR)

# 3. Convert the enhanced image to grayscale
gray_image = cv2.cvtColor(enhanced_image, cv2.COLOR_BGR2GRAY)

# 4. Denoise the grayscale image using GaussianBlur
blurred_image = cv2.GaussianBlur(gray_image, (5, 5), 0)

# 5. Apply adaptive thresholding for better results on uneven lighting
